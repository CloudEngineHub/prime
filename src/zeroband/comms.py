...
class ElasticDeviceMesh:
    ...
    def maybe_reinit_global_pg(self):
        ...
        self.live_recovery.check_live_recovery()
...
class LiveRecovery:
    ...
    # note(sami): right now the live reco is fixed and do not use tcp store.
    ...
    def __init__(self, global_store: dist.Store):
        ...
        self.live_ckpt_store = dist.PrefixStore("live_ckpt", self.global_store)

        self._should_send = self.world_info.global_rank == 0  # manual for now
        self._should_receive = self.world_info.global_rank == 1  # manual for now

        value = "1" if self._should_receive else "null"
        self.live_ckpt_store.set(f"need_live_ckpt_{self.world_info.global_rank}", value)

    def start_live_ckpt(self) -> int | None:
    ...
    def need_live_ckpt(self) -> bool:
        """Return True if the node need a live ckpt"""
        ...
        return self._should_receive

    def get_live_ckpt_src(self) -> int:
        """Return the rank to send the live ckpt to"""
        ...
        return self.world_info.global_rank - 1

    def check_live_recovery(self) -> None:
        """Check if the node need to send its weights to the next node"""
        ...
        # value = self.live_ckpt_store.get(f"need_live_ckpt_{self.world_info.global_rank + 1}").decode("utf-8")
        # self._logger.debug(f"need_live_ckpt_{self.world_info.global_rank + 1} = {value}")
        # if value == "1":
        #     self._should_send = True
        #     self.live_ckpt_store.set(f"need_live_ckpt_{self.world_info.global_rank + 1}", "null")
